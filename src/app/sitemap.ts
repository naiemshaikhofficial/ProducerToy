...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  const staticRoutes: MetadataRoute.Sitemap = [
    ...
    // Institutional Pages
    { url: `${baseUrl}/licensing`, lastModified: new Date(), changeFrequency: 'monthly', priority: 0.5 },
  ...
  try {
    ...
    const [productsRes, categoriesRes, subcategoriesRes, brandsRes] = await Promise.all([
      supabase.from('products').select('slug, updated_at, created_at').eq('is_active', true),
      supabase.from('categories').select('slug, created_at'),
      supabase.from('subcategories').select('slug, created_at'),
      supabase.from('brands').select('slug, created_at'),
    ])

    // 3. Dynamic Products URLs (Priority: 0.95)
