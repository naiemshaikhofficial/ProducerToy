...
export function CartPageClient() {
  ...
  const couponDiscountInr = Math.round(couponDiscountUsd * (exchangeRate || 95.0))
  ...
  const handleApplyCoupon = async (e: React.FormEvent) => {
    ...
    try {
      const res = await validateCouponAction(coupon.trim())
      if (res.success) {
      ...
      <div className="max-w-[1240px] mx-auto px-4 sm:px-6 lg:px-8 pt-8 sm:pt-12">
        ...
        {items.length === 0 ? (
          <div className="py-24 sm:py-32 flex flex-col items-center justify-center text-center space-y-6 animate-in fade-in duration-200">
            ...
            <div className="space-y-2 max-w-md">
              ...
              <p className="text-sm text-zinc-400">
                Explore our catalog of plugins, sample packs, presets, and DAW templates.
              </p>
            </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-10 pt-8 sm:pt-10 items-start">
                    ...
                    <div className="flex-1 flex flex-col justify-between space-y-4">
                      ...
                      <div className="flex items-start justify-between gap-4">
                        <div className="space-y-1.5">
                          {/* Category Tag Badge */}
                          <span className="inline-block bg-[#242424] text-zinc-300 text-[10.5px] font-bold uppercase tracking-wider px-2 py-0.5 rounded-md border border-[#303030]">
                            {categoryLabel}
                          </span>
                      ...
                      {/* Middle Metadata (Rating / License + Toywards Note + Refund Notice) */}
                      <div className="space-y-2.5 pt-1 border-t border-[#222222]">
                        
                        {/* Rating / Compatibility Box + Toywards Note */}
                        <div className="flex flex-wrap items-center gap-3 text-xs">
                          <span className="inline-flex items-center gap-1 font-bold bg-[#242424] text-zinc-300 border border-[#333333] px-2 py-0.5 rounded text-[11px]">
                            <ShieldCheck className="w-3 h-3 text-[#FA742B]" />
                            <span>100% Royalty-Free</span>
                          </span>

                          {/* Toywards Rewards Note (NO 5% mention) */}
                          <div className="inline-flex items-center gap-1.5 text-zinc-300 font-medium">
                        ...
                        {/* Self-Refundable Notice */}
                        <div className="flex items-center gap-1.5 text-[11.5px] text-zinc-400">
                          <span>Self-Refundable</span>
                          <span title="Subject to ProducerToy standard purchase and digital licensing policy">
                      ...
                      <div className="flex items-center justify-end gap-3 pt-2">
                        ...
                        <button
                          ...
                          className="p-2 rounded-xl bg-[#222222] hover:bg-[#2a2a2a] text-zinc-300 hover:text-white border border-[#303030] transition-colors cursor-pointer"
                        ...
                        <button
                          ...
                          className="px-4 py-2 rounded-xl bg-[#222222] hover:bg-[#2a2a2a] text-zinc-200 hover:text-white border border-[#303030] text-xs font-bold transition-colors cursor-pointer flex items-center gap-1.5"
                        >
            ...
            {/* --------------------------------------------------------------------- */}
            {/* RIGHT COLUMN: Games and Apps Summary (Sticky 4-5 cols)                */}
            {/* --------------------------------------------------------------------- */}
            <div className="lg:col-span-4 sticky top-24 space-y-4">
              ...
              <div className="bg-[#181818] border border-[#242424] rounded-2xl p-6 sm:p-7 space-y-6 shadow-xl">
                ...
                <div className="space-y-3 text-sm">
                  ...
                  {discountPercent > 0 && (
                    <div className="flex items-center justify-between text-[#FA742B] font-semibold text-xs animate-in fade-in">
                  ...
                  {/* Subtotal Divider */}
                  <div className="border-t border-[#242424] pt-3.5 flex items-center justify-between text-base font-bold">
                    <span className="text-white">Subtotal</span>
                    <span className="text-xl font-black text-white tracking-tight">
                      {formatPrice(finalTotalInr, finalTotalUsd)}
                    </span>
                  </div>
                ...
                <form onSubmit={handleApplyCoupon} className="space-y-2 pt-2 border-t border-[#242424]">
                  <div className="flex gap-2">
                    <input
                      ...
                      className="w-full bg-[#141414] border border-[#2c2c2c] focus:border-[#FA742B] text-white font-mono text-xs uppercase px-3.5 py-2.5 rounded-xl outline-none placeholder:text-zinc-600 transition-colors"
                    />
                    <button
                      type="submit"
                      disabled={applyingCoupon || !coupon.trim()}
                      className="bg-[#242424] hover:bg-[#303030] text-white border border-[#333333] font-bold text-xs px-4 rounded-xl transition-colors cursor-pointer disabled:opacity-50"
                    >
                      {applyingCoupon ? '...' : 'Apply'}
                    </button>
                  </div>
                  {couponError && (
                    <p className={`text-[11px] font-medium ${discountPercent > 0 ? 'text-green-400' : 'text-zinc-400'}`}>
                      {couponError}
                    </p>
                  )}
                </form>
                ...
                <div className="pt-2">
                  <button
                    ...
                    className="w-full bg-[#FA742B] hover:bg-[#E05A18] text-white font-black text-sm sm:text-base py-4 rounded-xl uppercase tracking-wider transition-all shadow-lg shadow-[#FA742B]/20 active:scale-[0.98] cursor-pointer flex items-center justify-center gap-2"
                  >
                    <span>Check Out</span>
